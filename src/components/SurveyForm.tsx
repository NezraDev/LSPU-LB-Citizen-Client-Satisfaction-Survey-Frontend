...
const SurveyForm: React.FC<SurveyFormProps> = ({
  ...
  const { personalInfoQuestions, ccQuestions, servicesList, qualityQuestions } =
    useMemo(() => {
      return {
        personalInfoQuestions: questions.filter(
          (q) => q.section_name.toLowerCase() === "personal info",
        ),
        ccQuestions: questions.filter(
          (q) => q.section_name.toLowerCase() === "citizens charter questions",
        ),
        servicesList:
          questions.find(
            (q) => q.section_name.toLowerCase() === "services attained",
          )?.options || [],
        qualityQuestions: questions.filter(
          (q) => q.section_name.toLowerCase() === "services quality dimensions",
        ),
        commentQuestion: questions.find(
          (q) => q.section_name.toLowerCase() === "comments and suggestions",
        ),
      };
    }, [questions]);
  ...
  const {
    ...
    isSuccessOpen,
    setIsSuccessOpen,
  } = useSurveyForm(questions, qrToken);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-lg p-4 sm:p-6 md:p-8">
        <SurveyHeader officeName={officeName} />

        <form
          onSubmit={(e) => {
            e.preventDefault();
            handleSubmit({ ...formData, qualityMap });
          }}
